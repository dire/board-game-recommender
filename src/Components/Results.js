          ...
          <div className="results">
            <Grid container spacing={4} alignItems="flex-start">
              {items && items.map((item, index) => (
                <Grid item className="result-item" xs={12} md={6} lg={4} key={item.id}>
                  <Grid container>
                    <Grid item xs={12}>
                      <Box className="game-thumbnail" p={2} textAlign={{ xs: 'center' }}>
                      ...
                      <Box mt={2}>
                        <Paper>
                          <Box className="game-details" display="flex" py={1} px={2}>
                            ...
                            {item.primary_designer ? <GameDetail title={"Designer"} detail={item.primary_designer.name} /> : null }
